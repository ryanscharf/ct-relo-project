...
## loop through towns and use osm API to get nodes tagged as footway or cycleway
 # for (i in 1:length(towns)) {
  ...
  path_count_fn <- function(x) {
    ...
    return(c(nodes = temp_node_count))
  }

  definitely_get_data = function(func, n_tries = 6, sleep = 1, ...){
  ...
  osm_results <- furrr::future_map(location, 
                                   .f = ~definitely_get_data(path_count_fn(.x))) %>%
    bind_rows()
  ...
  osm_results_j <- counties %>% 
   ...
   osm_results_j %>% write_csv('county_node_count.csv')
  beepr::beep(2)
  ...
  # agg datasets
  trail_df <- cbind(counties, osm_results)
