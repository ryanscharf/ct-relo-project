...
median_home_value_co %>% left_join(counties, by = 'GEOID') %>% st_sf() %>% 
...
# it turns out a lot of counties are missing from acs1
# we'll pull acs 5, get the relative change, then fill
# in the missing acs1 values with the asc5 values
# then juice it by the nearest % change from values
# that exist in both 
...
mhv <- median_home_value_co %>%
  right_join(median_home_value_co2, by = 'GEOID') %>% 
...
nearest_acs1 <- st_join(median_home_value_co_5yr %>% 
  ...
  select(GEOID = GEOID.x, estimate, inc)
...
# need to nearestneighbors  dg_n, hiking_trail_odes1
